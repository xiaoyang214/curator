...
public class InterProcessSemaphoreV2 {
    ...
    private InternalAcquireResult internalAcquire1Lease(ImmutableList.Builder<Lease> builder, long startMs, boolean hasWait,
        ...
        if (hasWait) {
            ...
            // 现获取 InterProcessMutex 锁， /lockPath/locks
            lock.acquire();
        }
        ...
        try {
            ...
            // /lockPath/lease
            String path = (nodeData != null) ? createBuilder
                    .forPath(ZKPaths.makePath(leasesPath, LEASE_BASE_NAME), nodeData) : createBuilder
            ...
            try {
                synchronized (this) {
                    for (; ; ) {
                        ...
                        // 如果 lease 目录下的节点个数 <= 3, 就会 break, 如果超出了，则触发 wait 操作
